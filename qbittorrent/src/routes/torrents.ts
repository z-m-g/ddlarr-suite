...
function toQBTorrentInfo(download: Download): QBTorrentInfo {
  ...
  return {
    ...
    ratio_limit: download.state === 'completed' ? 1 : -1,  // 1 = target ratio (already exceeded with ratio=999)
    seeding_time_limit: download.state === 'completed' ? 0 : -1,  // 0 minutes = immediately met
